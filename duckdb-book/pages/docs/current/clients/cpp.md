...
description: Installation To use the DuckDB C++ API, download the libduckdb archive
  ...
  }}. Warning DuckDB's C++ API is internal. It is not guaranteed to be stable and
  can change without notice. If you would like to build an application on DuckDB,
  we recommend using the C API. Installation The DuckDB C++ API can be installed as
  part of the libduckdb packages. Please see the installation page for details. Basic
...
1. Source page: https://duckdb.org/docs/current/clients/cpp
