---
type: Web Page
title: Safe Mode – DuckDB
description: 'The DuckDB CLI client supports “safe mode”. In safe mode, the CLI is
  prevented from accessing external files other than the database file that it was
  initially connected to and prevented from interacting with the host file system.
  This has the following effects: The following dot commands are disabled: .cd .excel
  .import .log .once .open .output .read .sh .system Auto-complete no longer scans
  the file system for files to suggest as auto-complete targets. The getenv function
...
- The following [dot commands](/docs/current/clients/cli/dot_commands.html)are disabled:- `.cd`
- `.excel`
- `.import`
- `.log`
- `.once`
...
- The `getenv`function
- The `enable_external_access`option`false`. This implies that:- `ATTACH`cannot attach to a database in a file.
...
1. Source page: https://duckdb.org/docs/current/clients/cli/safe_mode
