---
type: Web Page
title: Command Line Client – DuckDB
description: Installation To use the DuckDB CLI client, visit the CLI installation
  page. The latest stable version of the DuckDB command line client is {{ site.current_duckdb_version
  }}. Installation The DuckDB CLI (Command Line Interface) is a single, dependency-free
  executable. It is precompiled for Windows, Mac and Linux for both the stable version
  and for nightly builds produced by GitHub Actions. Please see the installation page
  under the CLI tab for download links. The DuckDB CLI is based on the SQLite command
...
The DuckDB CLI (Command Line Interface) is a single, dependency-free executable. It is precompiled for Windows, Mac and Linux for both the stable version and for nightly builds produced by GitHub Actions. Please see the [installation page](/install/) under the CLI tab for download links.
...
### 
        ...
        [In-Memory vs. Persistent Database](#in-memory-vs-persistent-database)
        
      

    
When no `FILENAME` argument is provided, the DuckDB CLI will open a temporary [in-memory database](/docs/current/connect/overview.html#in-memory-database).
...
D
```
To open or create a [persistent database](/docs/current/connect/overview.html#persistent-database), simply include a path as a command line argument:
...
### 
        ...
        [Running SQL Statements in the CLI](#running-sql-statements-in-the-cli)
...
The CLI supports all of DuckDB's rich [SQL syntax](/docs/current/sql/introduction.html) including `SELECT`, `CREATE` and `ALTER` statements.
...
The CLI supports [autocompletion](/docs/current/clients/cli/autocomplete.html), and has sophisticated [editor features](/docs/current/clients/cli/editing.html) and [syntax highlighting](/docs/current/clients/cli/syntax_highlighting.html) on macOS, Linux and Windows.
...
In addition to SQL syntax, special [dot commands](/docs/current/clients/cli/dot_commands.html) may be entered into the CLI client. To use one of these commands, begin the line with a period (`.`) immediately followed by the name of the command you wish to execute. Additional arguments to the command are entered, space separated, after the command. If an argument must contain a space, either single or double quotes may be used to wrap that parameter. Dot commands must be entered on a single line, and no whitespace may occur before the period. No semicolon is required at the end of the line.
...
#### 
        ...
        [Opening Database Files](#opening-database-files)
...
The `.mode` [dot command](/docs/current/clients/cli/dot_commands.html#mode) may be used to change the appearance of the tables returned in the terminal output.
...
For details, see the documentation for the [output dot command](/docs/current/clients/cli/dot_commands.html#output-writing-results-to-a-file).

#### 
        
        [Reading SQL from a File](#reading-sql-from-a-file)
...
### 
        ...
        [Setting a Custom Prompt](#setting-a-custom-prompt)
...
LOAD fts;
```
For details, see the [Extension docs](/docs/current/extensions/overview.html).

## 
        
        [Reading from stdin and Writing to stdout](#reading-from-stdin-and-writing-to-stdout)
...
The output of the `getenv` function can be used to set [configuration options](/docs/current/configuration/overview.html). For example, to set the `NULL` order based on the environment variable `DEFAULT_NULL_ORDER`, use:
...
### 
        ...
        [Restrictions for Reading Environment Variables](#restrictions-for-reading-environment-variables)
...
The DuckDB CLI supports executing [prepared statements](/docs/current/sql/query_syntax/prepared_statements.html) in addition to regular `SELECT` statements.
To create and execute a prepared statement in the CLI client, use the `PREPARE` clause and the `EXECUTE` statement.

## 
        
        [Query Completion ETA](#query-completion-eta)
...
### 
        ...
        [Factors Affecting The Accuracy of Query Completion ETA](#factors-affecting-the-accuracy-of-query-completion-eta)
...
1. Source page: https://duckdb.org/docs/current/clients/cli/overview
