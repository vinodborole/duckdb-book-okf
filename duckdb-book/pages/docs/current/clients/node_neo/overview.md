---
type: Web Page
title: Node.js Client (Neo) – DuckDB
description: 'Installation To use the DuckDB Node.js client, visit the Node.js installation
  page. The latest stable version of the DuckDB Node.js (Neo) client is {% if site.current_duckdb_node_neo_version
  != "" %}{{ site.current_duckdb_node_neo_version }}{% else %}{{ site.lts_duckdb_node_neo_version
  }}{% endif %}. An API for using DuckDB in Node.js. The primary package, @duckdb/node-api,
  is a high-level API meant for applications. It depends on low-level bindings that
  adhere closely to DuckDB''s C API, available separately as @duckdb/node-bindings.
  Roadmap Some features are not yet complete: Binding and appending the MAP and UNION
...
1. Source page: https://duckdb.org/docs/current/clients/node_neo/overview
