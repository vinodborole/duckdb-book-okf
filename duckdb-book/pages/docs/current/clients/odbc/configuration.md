...
This page documents the files using the ODBC configuration, [ odbc.ini](#odbcini-and-odbcini) and 

[. These are either placed in the home directory as dotfiles (](#odbcinstini-and-odbcinstini)

`odbcinst.ini``.odbc.ini` and `.odbcinst.ini`, respectively) or in a system directory.
For platform-specific details, see the pages for [Linux](/docs/current/clients/odbc/linux.html),

[macOS](/docs/current/clients/odbc/macos.html), and

[Windows](/docs/current/clients/odbc/windows.html).
...
`odbc.ini` and `.odbc.ini`The `odbc.ini` file contains the DSNs for the drivers, which can have specific knobs.
...
`odbcinst.ini` and `.odbcinst.ini`The `odbcinst.ini` file contains general configurations for the ODBC installed drivers in the system.
