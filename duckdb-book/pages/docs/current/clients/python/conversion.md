---
type: Web Page
title: Conversion between DuckDB and Python – DuckDB
description: 'This page documents the rules for converting Python objects to DuckDB
  and DuckDB results to Python. Object Conversion: Python Object to DuckDB This is
  a mapping of Python object types to DuckDB Logical Types: None → NULL bool → BOOLEAN
  datetime.timedelta → INTERVAL str → VARCHAR bytearray → BLOB memoryview → BLOB decimal.Decimal
  → DECIMAL / DOUBLE uuid.UUID → UUID The rest of the conversion rules are as follows.
  int Since integers can be of arbitrary size in Python, there is not a one-to-one
...
## 
        ...
        [Object Conversion: Python Object to DuckDB](#object-conversion-python-object-to-duckdb)
...
└─────────────────────────┘
```
If the dict is returned by a [function](/docs/current/clients/python/function.html), 
...
```
If the dict is returned by a [function](/docs/current/clients/python/function.html), 
the function will return a `MAP`, due to [automatic conversion](/docs/current/clients/python/types.html#dictkey_type-value_type).
...
## 
        ...
        [Result Conversion: DuckDB Results to Python](#result-conversion-duckdb-results-to-python)
...
Below are some examples using this functionality. See the [Python guides](/docs/current/guides/overview.html#python-client) for more examples.
...
1. Source page: https://duckdb.org/docs/current/clients/python/conversion
