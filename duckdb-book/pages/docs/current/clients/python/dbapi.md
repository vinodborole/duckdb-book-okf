---
type: Web Page
title: Python DB API – DuckDB
description: 'The standard DuckDB Python API provides a SQL interface compliant with
  the DB-API 2.0 specification described by PEP 249 similar to the SQLite Python API.
  Connection To use the module, you must first create a DuckDBPyConnection object
  that represents a connection to a database. This is done through the duckdb.connect
  method. The ''config'' keyword argument can be used to provide a dict that contains
  key->value pairs referencing settings understood by DuckDB. In-Memory Connection
...
#### 
        ...
        [Named In-memory Connections](#named-in-memory-connections)
...
### 
        ...
        [File-Based Connection](#file-based-connection)
...
[('laptop',), ('chainsaw',)]
```
Query using the `$` notation for a [prepared statement](/docs/current/sql/query_syntax/prepared_statements.html) and reused values:
...
1. Source page: https://duckdb.org/docs/current/clients/python/dbapi
