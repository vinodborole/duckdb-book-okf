---
type: Web Page
title: Import Data – DuckDB
description: 'Overview The JDBC driver offers two ways to load data into DuckDB in
  bulk: the high-performance Appender and a JDBC batch writer. This page describes
  both and when to use each. Appender The Appender is available in the DuckDB JDBC
  driver via the org.duckdb.DuckDBAppender class. An appender is created from a DuckDBConnection
  with the createAppender() method, passing the table name and, optionally, the schema
  and catalog it is applied to. The Appender is flushed when the close() method is
  called. Example: import java.sql.DriverManager; import org.duckdb.DuckDBConnection;
  DuckDBConnection conn = (DuckDBConnection) DriverManager.getConnection("jdbc:duckdb:");
  try (var stmt = conn.createStatement()) { stmt.execute("CREATE TABLE tbl (x…'
resource: https://duckdb.org/docs/current/clients/java/data_import
...
### 
        ...
        [Maximizing Appender Performance](#maximizing-appender-performance)
...
### 
        ...
        [Single-Value Appender (Legacy)](#single-value-appender-legacy)
...
- [Appender](/docs/current/data/appender.html) — the engine-level Appender interface that the Java`DuckDBAppender` wraps.
- [Data Import](/docs/current/data/overview.html) — DuckDB's other bulk-loading options, including reading directly from CSV, Parquet, and JSON files.
- [Run Queries](/docs/current/clients/java/querying.html) — sending the[`INSERT`](/docs/current/sql/statements/insert.html) and[`CREATE TABLE`](/docs/current/sql/statements/create_table.html) statements the batch writer builds on.
- [Define Connections](/docs/current/clients/java/connecting.html) — obtaining the`DuckDBConnection` an Appender is created from.

# Citations

1. Source page: https://duckdb.org/docs/current/clients/java/data_import
