---
type: Web Page
title: Profile and Monitor – DuckDB
description: 'Overview The JDBC driver exposes several ways to observe a running or
  completed query: cancelling and timing out queries, polling query progress, retrieving
  profiling output, and emitting memory-usage events through Java Flight Recorder.
  This page covers each of them. Query Progress and Cancellation A running query can
  be cancelled from another thread with the standard JDBC Statement.cancel() method,
  which interrupts the query executing on the statement''s connection. Setting Statement.setQueryTimeout(seconds)
  arms an automatic cancellation after the given number of seconds. For long-running
  queries, DuckDBPreparedStatement.getQueryProgress() returns a QueryProgress snapshot
  while the query executes. It is intended to be called from a separate…'
resource: https://duckdb.org/docs/current/clients/java/profiling
...
## 
        ...
        [Query Progress and Cancellation](#query-progress-and-cancellation)
...
## 
        ...
        [Memory Monitoring with JFR](#memory-monitoring-with-jfr)
...
### 
        ...
        [Enabling Emission](#enabling-emission)
...
### 
        ...
        [Controlling the Period and Enabled State](#controlling-the-period-and-enabled-state)
...
Whether two `getConnection` calls share a `dbAddress` follows the same rules as [instance caching](/docs/current/clients/java/connecting.html#database-instances-and-instance-caching):
...
- [Define Connections](/docs/current/clients/java/connecting.html) — the`jdbc_jfr_memory_monitor` property and the query-cancel scheduler thread the driver runs.
- [Profiling](/docs/current/dev/profiling.html) — DuckDB's query profiling output and the formats`getProfilingInformation()` returns.
- [Run Queries](/docs/current/clients/java/querying.html) — running the queries whose progress, timing, and profiles this page observes.

# Citations

1. Source page: https://duckdb.org/docs/current/clients/java/profiling
