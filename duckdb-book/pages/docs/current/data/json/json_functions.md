---
type: Web Page
title: JSON Processing Functions – DuckDB
description: JSON Extraction Functions There are two extraction functions, which have
  their respective operators. The operators can only be used if the string is stored
  as the JSON logical type. These functions support the same two location notations
  as JSON Scalar functions. Function Alias Operator Description json_exists(json,
  path)     Returns true if the supplied path exists in the json, and false otherwise.
  json_extract(json, path) json_extract_path -> Extracts JSON from json at the given
...
1. Source page: https://duckdb.org/docs/current/data/json/json_functions
