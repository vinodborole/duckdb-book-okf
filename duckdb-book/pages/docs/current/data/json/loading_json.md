...
In the following example, we use the `todos.json` file,
...
| `timestampformat` | Specifies the date format to use when parsing timestamps. See Date Format. When set to `iso`(the default), ISO 8601 timestamps with timezone offsets (e.g.,`2024-01-01T12:00:00+05:00`) and fractional seconds (e.g.,`2024-01-01T12:00:00.123Z`) are automatically inferred as`TIMESTAMP`. | `VARCHAR` | `iso` | 
| `union_by_name` | Whether the schemas of multiple JSON files should be unified | `BOOL` | `false` | 
