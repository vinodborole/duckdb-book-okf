...
`INSERT` Statements`INSERT` statements are the standard way of loading data into a database system. They are suitable for quick prototyping, but should be avoided for bulk loading as they have significant per-row overhead.
...
`file_search_path` is not set, the working directory is used as the basis for relative paths.## 
