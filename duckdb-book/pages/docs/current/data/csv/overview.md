...
CREATE TABLE ontime AS
    SELECT * FROM 'flights.csv';
```
We can use the [`FROM`-first syntax](/docs/current/sql/query_syntax/from.html#from-first-syntax) to omit `SELECT *`.
...
Below are parameters that can be passed to the [`read_csv` function](#csv-functions). Where meaningfully applicable, these parameters can also be passed to the [`COPY` statement](/docs/current/sql/statements/copy.html#copy-to).
...
For other encodings, you can either use [the `encodings` extension](/docs/current/core_extensions/encodings.html)
...
The `auto_type_candidates` option lets you specify the data types that should be considered by the CSV reader for [column data type detection](/docs/current/data/csv/auto_detection.html#type-detection).
Usage example:
