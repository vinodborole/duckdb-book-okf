...
To use DuckDB, you must first create a connection to a database. The exact syntax varies between the [client APIs](/docs/current/clients/overview.html) but it typically involves passing an argument to configure persistence.
...
Starting with v0.10, DuckDB's storage format is [backwards-compatible](/docs/current/internals/storage.html#backward-compatibility), i.e., DuckDB is able to read database files produced by an older version of DuckDB.
For example, DuckDB v0.10 can read and operate on files created by the previous DuckDB version, v0.9.
For more details on DuckDB's storage format, see the [storage page](/docs/current/internals/storage.html).
...
1. Source page: https://duckdb.org/docs/current/connect/overview
